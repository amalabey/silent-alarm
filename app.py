...
def get_alarms():
    ...
    alarm_rows = conn.execute('SELECT id, alarmtime, alarmstate, tag FROM alarms').fetchall()
    alarms_list = list()
    for row in alarm_rows:
        if row['alarmtime'] is not None:
            alarm = { 'id': row['id'], 'time': row['alarmtime'], 'timedt': datetime.strptime(row['alarmtime'],"%Y-%m-%d %H:%M"), 'state': row['alarmstate'], 'tag': row['tag']}
...
def add_alarm():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('INSERT INTO alarms (alarmtime, tag) VALUES(?, ?)', (request.json['alarmtime'], tag))
    conn.commit()
    conn.close()
